...
for i in range(0, num_runs):
    ...
    for e in range(0, len(input_feed)):
        ...
        loa = [a for a in inp_vectors.values()]
        vectors = np.concatenate( loa, axis=0 )
...
if 1:
    ...
    vectors_ = model.fit_transform(vectors)

    normalizer = preprocessing.Normalizer()
    vectors_ =  normalizer.fit_transform(vectors_, 'l2')

    fig, ax = plt.subplots()
    for i in range(len(vocab)):
        print(vocab[i], vectors_[i][1])
        ax.annotate(vocab[i], (vectors_[i][0],vectors_[i][1]))
    plt.xlim([-2,2])
    plt.ylim([-2,2])
    plt.show()

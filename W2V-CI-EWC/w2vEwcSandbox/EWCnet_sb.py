...
class Net:
    def __init__(self, x, y_):
        ...
        W1 = weight_variable([in_dim,numNodes])
        b1 = bias_variable([numNodes])

        W2 = weight_variable([numNodes,out_dim])
    ...
    def compute_fisher(self, imgset, sess, num_samples=200, plot_diffs=False, disp_freq=10):
        # Meat and potatos: compute Fisher information for each parameter
        ...
        for i in range(num_samples): # aka do this 200 times, could increase...
            # select random input image
            im_ind = np.random.randint(imgset.shape[0])
            # compute first-order derivatives
            # uses the validation images in mnist
    ...
    def set_vanilla_loss(self):
        # this uses SGD to train the net
    ...
    def update_ewc_loss(self, lam):
        ...
        if not hasattr(self, "ewc_loss"): # check if it has ran
            self.ewc_loss = self.cross_entropy

        #  for each weight and bias
        #     multiply the fisher information by the squared difference of the current weights, and the last tasks weights
        #     Then retrain the net to minimize this new loss function. 
        for v in range(len(self.var_list)):
